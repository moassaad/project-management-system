...
export const createTaskSchema = z.object({
  title: z.string().min(1, 'Task title is required').max(255, 'Title must be at most 255 characters'),
  ...
  dueDate: z
    ...
    .optional()
    .or(z.literal(''))
    .refine((v) => !v || !isNaN(Date.parse(v)), 'Due date must be a valid ISO date'),
})

export const updateTaskSchema = z.object({
  title: z.string().min(1, 'Task title is required').max(255, 'Title must be at most 255 characters').optional(),
  description: z.string().max(255, 'Description must be at most 255 characters').optional().or(z.literal('')).nullable(),
  ...
  dueDate: z
    ...
    .or(z.literal(''))
    .nullable()
    .refine((v) => !v || !isNaN(Date.parse(v as string)), 'Due date must be a valid ISO date'),
})
